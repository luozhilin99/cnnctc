...
cnnctc.ckpt can be downloaded [**HERE**](https://download.mindspore.cn/models/r1.8/cnnctc_ascend_v180_mjstiiit_official_cv_acc84.0.ckpt).

```bash
python quick_start.py --device_target CPU --CHECKPOINT_PATH ckpt/cnnctc.ckpt --IMG_PATH imgs/1.png
```
...
|--- CNNCTC/
    ...
    |---train_cpu.py    // train cpu scripts
...
- Standalone CPU Training:

```bash
python train_cpu.py --device_target="CPU" --PRED_TRAINED="" --run_distribute=False
```

Results and checkpoints are written to `./` folder and loss values are recorded in `./loss.log`. (Only supports run-through)
...
```bash
python eval.py --device_target="CPU" --CHECKPOINT_PATH=""
```
